...
contract Controller is Ownable {
  ...
  modifier onlyAdmin() {
    if(!(isAdmin[_msgSender()] || owner() == _msgSender())) revert UnAthourizedAccess();
        _;
    }
  ...
  function addAdmin(address _admin , bool add) public onlyOwner {
      if (add) {
          if(isAdmin[_admin] ) revert AlreadyRegistered();
          emit AdminAdded(_admin);
          admins.push(_admin);
      } else {
          if(!isAdmin[_admin] ) revert NotRegistered();
  ...
  function addRegistrar(address _registrar , bool add) external onlyAdmin {
    ...
    function addOracle(address _oracle , bool add) external onlyAdmin {
        if (add) {
            if(isOracle[_oracle]) revert AlreadyRegistered();
            emit OracleAdded(_oracle);
            oracles.push(_oracle);
        } else {
        if(!isOracle[_oracle] ) revert NotRegistered();
   ...
   function addValidator(address _validator , bool add) external onlyAdmin {
        if (add) {
            if(isValidator[_validator]) revert AlreadyRegistered();
            emit ValidatorAdded(_validator);
            validators.push(_validator);
        } else {
            if(!isValidator[_validator]) revert NotRegistered();
